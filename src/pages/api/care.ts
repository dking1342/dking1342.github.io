...
export default function handler(
  ...
  const result = list.filter((val) => {
    if (req.query.id) {
      return val.id === Number(req.query.id);
    } else {
      return val;
    }
  });
